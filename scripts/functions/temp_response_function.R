Thermal_tol2 <- function(fv_fm, temperature, individual, data = NULL, print_graph = TRUE, graph_dir) {
  ...
  data <- data.frame(individual, fv_fm, temperature)  # new data frame to perform the analysis
  
  # Objects to store results across all individuals
  
  ID = Fv_Fm_Max = ctmax = ctmax.lci = ctmax.uci = tcrit = tcrit.lci = tcrit.uci = t15 = t15.lci = t15.uci = R2 = c()
  ...
  for (t in 1:length(id_levels)) {
    ...
    x <- seq(from = min(datum2$temperature), to = max(datum2$temperature))
    lx <- length(x)
    y <- phia / (1 + exp(-(phib + phic * x)))
    predict <- data.frame(x, y)
    
    # calculates half of the initial Fv/Fm (T50 target)
    ...
    ctmax.half.bs = Tcrit.bs = T15.bs = c()
    ...
    for (k in 1:100) {  # start loop for bootstrap estimation
      ...
      if (class(try(nls(fv_fm ~ theta1 / (1 + exp(-(theta2 + theta3 * temperature))),
        ...
        x2 <- seq(from = min(datum2$temperature), to = max(datum2$temperature), length.out = 41)
        y2 <- phia2 / (1 + exp(-(phib2 + phic2 * x2)))
        
        predict.boot[, k] <- y2
        ctmax.fl1 <- (-log((phia2 / half) - 1) - phib2) / phic2  # estimate ctmax (T50)
        ctmax.half.bs[k] <- ctmax.fl1  # estimate ctmax
        
        t15.fl1 <- (-log((phia2 / fifteen) - 1) - phib2) / phic2  # estimate T15
        T15.bs[k] <- t15.fl1  # estimate T15
    ...
    t15[t] <- round(mean(na.omit(T15.bs)), 2)  # predicted T15 (15% reduction)
    t15.lci[t] <- round(na.omit(T15.CI[1]), 2)  # T15 lower confidence interval
    t15.uci[t] <- round(na.omit(T15.CI[2]), 2)  # T15 upper confidence interval
    ...
    if (!print_graph) {  # start plotting action 1
      ...
      text(x = 55.8, y = 0.70, labels = bquote("T"[15] == ""))
      text(x = 61.1, y = 0.70, labels = paste(round(t15[t], digits = 2)))
      ...
      abline(v = round(mean(na.omit(T15.bs)), 2), lty = 2, lwd = 1.5, col = "steelblue")
